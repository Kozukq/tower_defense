...
void* soldier_behaviour(void* arg) {
	...
	minY = soldier_state->soldier->position[Y] - 1;
	if(minY < 0){
		minY = 0;
	}

	maxY = soldier_state->soldier->position[Y] + 1;
	if(maxY > 14){
		maxY = 14;
	}

	minX = soldier_state->soldier->position[X] - 1;
	if(minX < 0){
		minX = 0;
	}

	maxX = soldier_state->soldier->position[X] + 1;
	if(maxX > 14){
		maxX = 14;
	}
